...
def from_audio_to_table(uploaded_wav_file):
    data = extract_mfcc_features(uploaded_wav_file)
...
def predict_wav(uploaded_wav_file, learn, config):
    wave_df = from_audio_to_table(uploaded_wav_file)
    ...
    return probs.head()# wave_df_proc, clas, probs
...
if selection == "Inferencer":
    ...
    if uploaded_wav is not None:
        ...
        st.write(predict_wav(uploaded_wav, learn_inf, config))
